...
def initialize_sws_links():
    ...
    df_links = gpd.GeoDataFrame(df_links, geometry=df_links['geometry'], crs=df_links.crs)
    print(df_links)
    df_links['inter_id'] = df_links['inter_id'].fillna(-1)
...
def build_walk_network(df_links):
    ...
    :return: DGo: directed link graph.
    """
    df_links['walk_speed'] = df_links['speed']  # 3 mph
    # The measure to use as cost of traverse a link
    col = 'time'  # can be expand to other measures, like considering grades, etc.
    ...
    for ind, row2 in df_links.iterrows():
        DGo.add_weighted_edges_from([(str(row2['A']), str(row2['B']), float(row2[col]) * 60.0)],
                                    weight='forward', dist=row2['distance'] / 5280, name=row2['inter_id'])
...
def samp_pre_process(filename, dict_settings, option='sidewalk'):
...
def Bike_route_finder(row, option, dict_settings):
    ...
    plot_all = False # dict_settings['plot_all']
    if plot_all:
...
def plot_seq_sw(trip_seq, gpd_df, trip_series=None):
    """
    trip_seq: a list of link id (includes 'origin' and 'destination')
    gdf_df: Geopandas dataframe (shapefile) of abm15
    skeleton: a list of link ID. if not None, those will be plotted as skeleton traffic network (e.g., I-85).
...
def allRun(df_points, options, dict_settings, ind_start, n_ind):
    # return csv file names with datetime info
    from datetime import datetime
    now = datetime.now()  # used time to uniquely name outputs sub-folder
    now_str = now.strftime("%y%m%d_%H%M%S_{}".format(ind_start))
    new_folder = os.path.join(os.getcwd(), 'build_graph/results', now_str)
    ...
    trips = []
    for ind, row in df_points.iterrows():
        if ind < ind_start or ind > ind_start + n_ind:
            # print(ind)
            continue
        # print(f'{ind} aaa')
        ti = time.time()
        ...
        # 
        if ind % 10 == 9:
            logs.to_csv(os.path.join(new_folder, 'logs_walk_{}-{}.csv'.format(ind-9, ind)), index=False)
            logs = pd.DataFrame()
    # 
...
def main_BikewaySim(args):
    os.environ['PROJ_LIB'] = args.path
    ...
    allRun(df_points, options, dict_settings, args.s, args.n)


def main_SidewalkSim(args):
    os.environ['PROJ_LIB'] = args.path
    dirPath = os.path.join(os.environ['PROJ_LIB'], 'build_graph')
    os.chdir(dirPath)

    # Section 1: BUILD GRAPHS & ASSUMPTIONS
    networkInit = True ## Fizzy's Edit, Dec. 2021
    # try:
    ...
    # except:
    if networkInit: ## Fizzy's Edit, Dec. 2021
        df_links = gpd.read_file(os.path.join(os.environ['PROJ_LIB'], 'build_graph', \
                                'data_node_link', 'SWS_links.shp'))
    else: ## Fizzy's Edit, Dec. 2021
    ...
    if dict_settings['one_by_one'] is False:
        if sample_Prepared:
            ...
            for samp_file in glob.glob(os.path.join('samples_in', '*.csv')):
                df_points = df_points.append(samp_pre_process(samp_file, dict_settings, option='sidewalk'),
                                             ignore_index=True)
    else:
        if sample_Prepared:
            for samp_file in glob.glob(os.path.join('one_by_one_out', '*.csv')):
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Run SidewalkSim')
    parser.add_argument('-path', default='/storage/coda1/p-rg49/0/hfan63/CEE6701/SidewalkSim', \
        help='Path of sidewalksim folder')
    parser.add_argument('-s', type=int, default=0, help='Row start index')
    parser.add_argument('-n', type=int, default=2**32, help='Number of rows to run')
    parser.add_argument('--model', choices=['SidewalkSim','BikewaySim'], default='SidewalkSim',\
        help='Model to run, default=SidewalkSim')
    args = parser.parse_args()
    if args.model == 'SidewalkSim':
        main_SidewalkSim(args)
    else:
        main_BikewaySim(args)
